...
import { getNotionProjects, isNotionProjectPage } from '@/lib/lily-os/projects'
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  // 3. /article/<UUID> 这类路径按最后一段匹配数据库记录
  if (
    !post &&
    typeof lastSegment === 'string' &&
    /^[a-f0-9-]{32,36}$/i.test(lastSegment)
  ) {
    post = props?.allPages?.find(p => p?.id === idToUuid(lastSegment))
  }

  // 4. 最后一段是 UUID，直接拉 Notion
  if (
    !post &&
    typeof lastSegment === 'string' &&
    /^[a-f0-9-]{32,36}$/i.test(lastSegment)
  ...
  // 5. 拿到 post 但没有 blockMap，补拉 block
...
async function convertNotionToSiteData(
  ...
  return {
    ...
    pageIds,
    latestPosts,
    lilyProjects: getNotionProjects(collectionData)
  }
}
