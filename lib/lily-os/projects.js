...
export const isNotionProjectPage = page =>
  Boolean(
    page?.Project ||
      page?.project ||
      page?.['项目名称'] ||
...
export const getNotionProjects = allPages =>
  (allPages || [])
    .map(page => {
      const hasProjectMetadata = isNotionProjectPage(page)
      ...
      const isPublic = ['Published', 'Invisible'].includes(page.status)
      const href = isPublic
        ? page.type === 'Page' && page.id
          ? `/article/${page.id}`
          : page.href || ''
        : ''
